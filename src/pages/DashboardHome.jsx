...
const DashboardHome = () => {
    ...
    return (
        <div className="space-y-8 min-h-full">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {statCards.map((card, i) => (
                    ...
                    >
                        <MetricCard
                            title={card.title}
                            value={loading ? '—' : (card.key === 'timeSaved' ? stats.timeSaved : stats[card.key])}
                            icon={card.icon}
                            color={card.color}
                            bg={card.bg}
                            border={card.border}
            ...
            >
                <div key="stats1"><DashboardWidget id="stats1" title="Datasets" isDraggable={isEditing}><div className="flex flex-col"><Database className="h-7 w-7 text-blue-400 mb-2" /><p className="text-3xl font-black text-white">{loading ? '...' : stats.datasets}</p><p className="text-xs text-white/35">Connected sources</p></div></DashboardWidget></div>
                <div key="stats2"><DashboardWidget id="stats2" title="Intelligence" isDraggable={isEditing}><div className="flex flex-col"><Sparkles className="h-7 w-7 text-violet-400 mb-2" /><p className="text-3xl font-black text-white">{loading ? '...' : stats.insights}</p><p className="text-xs text-white/35">AI generated patterns</p></div></DashboardWidget></div>
                <div key="stats3"><DashboardWidget id="stats3" title="Charts" isDraggable={isEditing}><div className="flex flex-col"><BarChart3 className="h-7 w-7 text-cyan-400 mb-2" /><p className="text-3xl font-black text-white">{loading ? '...' : stats.charts}</p><p className="text-xs text-white/35">Active reports</p></div></DashboardWidget></div>
                <div key="stats4"><DashboardWidget id="stats4" title="Time Saved" isDraggable={isEditing}><div className="flex flex-col"><Clock className="h-7 w-7 text-emerald-400 mb-2" /><p className="text-3xl font-black text-white">{loading ? '...' : `${stats.timeSaved}h`}</p><p className="text-xs text-white/35">Manual effort reduced</p></div></DashboardWidget></div>
                        ...
                        <div className="space-y-3">
                            ...
                            {recentDatasets.length === 0 && !loading && (
                                <div className="text-center py-8">
                                    ...
                                    <p className="text-white/30 text-sm">No datasets yet.</p>
                                    <Link to="/dashboard/upload" className="inline-flex items-center gap-1 mt-3 text-xs font-bold text-violet-400 hover:text-violet-300 transition-colors">
                                        Upload your first dataset <ChevronRight className="h-3 w-3" />
                                    </Link>
                                </div>
                            )}
                        </div>
