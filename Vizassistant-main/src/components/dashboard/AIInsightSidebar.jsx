        ...
        <AnimatePresence>
            {isOpen && (
                <>
                    ...
                    >
                        <div className="p-6 border-b border-[#ACBBC6]/10 flex items-center justify-between bg-white/[0.02]">
                            <div className="flex items-center gap-3">
                                <div className="bg-primary/18 p-2 rounded-xl">
                        ...
                        <div className="flex-1 overflow-y-auto p-6 space-y-6">
                            {loading ? (
                                ...
                                <div className="space-y-6">
                                    <div className="glass-card bg-primary/8 border-primary/20 p-5 rounded-2xl relative overflow-hidden group">
                                        <div className="absolute top-0 right-0 p-2 opacity-10">
                                            <Zap className="h-12 w-12" />
                                        </div>
                                        <h4 className="text-xs font-black text-[#ACBBC6] uppercase tracking-widest mb-3 flex items-center gap-2">
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        {['Identify anomalies', 'Summarize key trends', 'Predict next period'].map((q) => (
                                            <button
                                                ...
                                                className="w-full text-left p-3 rounded-xl bg-white/[0.03] border border-white/6 hover:border-primary/25 hover:bg-white/[0.05] transition-colors text-xs text-white/60 font-medium flex items-center justify-between group"
                                            >
                                                {q}
                        ...
                        <div className="p-6 border-t border-[#ACBBC6]/10 bg-white/[0.01]">
                            <form onSubmit={handleQuickAsk} className="relative group">
                                <input
                                    ...
                                    className="w-full bg-white/[0.03] border border-white/10 rounded-2xl pl-5 pr-12 py-4 text-sm text-white placeholder:text-white/20 focus:ring-2 focus:ring-primary/40 focus:border-primary/40 transition-all"
                                />
                                <button
                                    type="submit"
                                    disabled={chatLoading || !chatQuery.trim() || !latestDataset}
                                    className="absolute right-2 top-1/2 -translate-y-1/2 p-2.5 bg-[#667D9D] text-[#060817] rounded-xl shadow-lg shadow-[#060817]/20 hover:scale-105 active:scale-95 transition-all disabled:opacity-50 disabled:pointer-events-none"
                                >
